...
* **Priority Queue:** dequeue highest priority (not strictly FIFO)
...
* Arrays teach **indexing and memory**
* Linked lists teach **pointers and dynamic allocation**
* Stacks teach **recursion and reversal**
* Queues teach **scheduling and order maintenance**
...
* **Linear probing:** try next slot (+1)
* **Quadratic probing:** step size increases quadratically
...
* Worst-case **O(1)** lookup (constant probes)
* Predictable latency

**Cons:**

* Rehash needed on insertion failure
* More complex logic
...
**Key Principle:**
[
\text{If new_probe_distance} > \text{existing_probe_distance} \Rightarrow \text{swap}
]
...
* **Distributed caches** (Memcached, DynamoDB)
* **Load balancing**
...
* **Max-Heap:** Every node ≥ its children
...
* **Heapsort**: Repeatedly pop min (O(n log n))
* **Priority Queue**: Fast access to smallest/largest
* **Graph Algorithms**: Dijkstra, Prim
* **Streaming**: Median finding using two heaps
...
* A circular linked list of roots
* Each node can have multiple children
* Consolidation on extract-min ensures minimal degree duplication
...
* `merge` two heaps: attach one as child of the other
* `extract-min`: remove root, merge children in pairs, then merge all results

**Why It’s Popular:**

* Easier to implement
* Great real-world performance
* Used in functional programming and priority schedulers
...
* **LL (Right Rotation)**: imbalance on left-left
* **RR (Left Rotation)**: imbalance on right-right
...
* Strict height bound: (O(\log n))
* More rotations (slower insertions)
...
* Fewer rotations (faster insert/delete)
* Slightly taller (slower lookup)
* Simpler amortized balance

**Used in:**

* C++ `std::map`, `std::set`
* Java `TreeMap`, Linux scheduler
...
* **Zig:** one rotation (root child)
* **Zig-Zig:** two rotations (same side)
* **Zig-Zag:** two rotations (different sides)
...
* Query over a range in (O(\log n))
* Update elements in (O(\log n))
...
* If current range [L, R] fully inside [l, r], return node value
* If disjoint, return 0
* Else combine children
...
* **Sum / Min / Max / XOR** queries
* **Frequency counts**
* **Inversions counting**
* **Order statistics**
* **Online problems** where array updates over time

Used in:

* Competitive programming
* Databases (analytics on changing data)
* Time series queries
* Games (damage/range updates)
...
* Merge two sets
* Check if two elements share the same set
...
* **Kruskal’s MST**: add edges, avoid cycles
...
* **Bloom Filters** → membership queries
...
* Caches (check before disk lookup)
* Spam filters
* Databases (join filtering)
...
* Compute all (h_i(x))
* Take `min(count[i][h_i(x)])` across rows
  → gives an **upper-bounded** estimate of true frequency
...
* **Prefix bits** → bucket index
* **Suffix bits** → count leading zeros
...
* **Skip Lists** → randomized, layered linked lists that behave like balanced BSTs
* **B-Trees** → multi-way trees that minimize disk I/O and organize large data blocks
...
* Drop down when you can’t go further
...
* Level 1 (base) always exists
* Level 2 with probability ½
* Level 3 with ¼, etc.
...
* Support **concurrent access** well
* Randomized, not deterministic — but highly reliable

Used in:

* Redis (sorted sets)
* LevelDB / RocksDB internals
* Concurrent maps
...
* Each node holds **multiple keys and children**
...
* Each internal node has `k-1` keys if it has `k` children
...
1. **Search**
   ...
   * Traverse from root
   * Binary search in each node’s key array
...
* All **data in leaves** (internal nodes = indexes)
...
* **Randomized simplicity** (Skip List)
* **Block-based order** (B-Tree)

Both offer logarithmic guarantees, but one optimizes **pointer chasing**, the other **I/O**.

They’re fundamental to:

* In-memory caches (Skip List)
* On-disk indexes (B-Tree, B+ Tree)
...
* Use **balanced binary trees** as array replacements
* Each update replaces one node
* Persistent **vector** = tree of small arrays (used in Clojure, Scala)
...
* Each update = new root
* Each version = snapshot of history
...
* **Version control** (Git trees)
* **Immutable databases** (Datomic)
* **Segment trees over time** (competitive programming)
* **Snapshots** in memory allocators or games
...
* **Order Statistic Trees** (k-th element, rank queries)
* **Interval Trees** (range overlaps)
* **Range Trees** (multi-dimensional search)
* **KD-Trees** (spatial partitioning)
* **Merge Sort Trees** (offline range queries)
...
* Databases (ORDER BY, pagination)
* Quantile queries
* Online median maintenance
...
* `max`: maximum `high` in its subtree
...
* Calendar/schedule conflict detection
* Collision detection
* Genome region lookup
...
* Primary BST on x
* Each node stores secondary BST on y
...
* **Iterative Deepening** — a memory-friendly hybrid
...
* **AI search problems** (state spaces)
* **Game trees** (chess, puzzles)
...
* DFS Tree: based on recursion
* BFS Tree: based on levels

Use them to:

* Detect **cross edges**, **back edges**
* Classify edges (important for algorithms like Tarjan’s)
...
* **Kosaraju’s Algorithm** — clean, intuitive, two-pass
* **Tarjan’s Algorithm** — one-pass, stack-based elegance
...
* **Program analysis:** detecting cycles, dependencies.
* **Web graphs:** find clusters of mutually linked pages.
* **Control-flow:** loops and strongly connected subroutines.
...
* A node is **root of an SCC** if `disc[u] == low[u]`
...
* **Single-source shortest path (SSSP)** — one source to all
* **Single-pair** — one source to one target
* **All-pairs shortest path (APSP)** — every pair
...
* Works only if all edges (w \ge 0)
* Can reconstruct path via `parent[v]`
* Used in:

  * GPS and routing systems
  * Network optimization
  * Scheduling with positive costs
...
* (g(v)): known cost so far
* (h(v)): heuristic estimate (must be admissible)
...
* **Bidirectional Search** → meet-in-the-middle for speed
* **Heuristic A*** → smarter exploration guided by estimates
...
* When relaxing an edge with weight 0, push to **front**.
* When relaxing an edge with weight 1, push to **back**.
...
* Layered BFS
* Grid problems with binary costs
* BFS with teleportation (weight 0 edges)
...
* Maze solvers
* Shortest paths in large sparse graphs
...
* (g(v)): cost so far
* (h(v)): estimated cost to goal
* (h(v)) must be **admissible** ((h(v) \le \text{true cost}))
...
* **Grid map**: Manhattan distance
  (h(x, y) = |x - x_g| + |y - y_g|)
* **Navigation**: straight-line (Euclidean)
* **Game tree**: evaluation function
...
* **Prim’s** — vertex-based, greedy expansion
* **Borůvka’s** — component merging in parallel
...
3. For each edge (u, v):
   ...
   * If `u` and `v` are in different components → include edge
   * Union their sets
...
* Union-Find operations: (O(\alpha(V))) (almost constant)
* Total: (O(E \log E))
...
* **Cut Property:** For any cut, smallest crossing edge ∈ MST.
* **Cycle Property:** For any cycle, largest edge not ∈ MST.
* MST may not be unique if equal weights.
...
* Path queries
* Clustering (remove largest edge)
* Approximation TSP (preorder traversal)
...
* **Ford–Fulkerson** — the primal idea
* **Edmonds–Karp** — BFS-based implementation
* **Dinic’s Algorithm** — layered speed
...
* Network routing
* Bipartite matching
* Task assignment (flows = people → jobs)
* Image segmentation (min-cut)
* Circulation with demands
* Data pipelines, max throughput systems
...
* **Stoer–Wagner** — deterministic min-cut for undirected graphs
* **Karger’s Randomized Algorithm** — fast, probabilistic
* **Gomory–Hu Tree** — compress all-pairs min-cuts into one tree
...
* **Network reliability** — weakest link detection
* **Clustering** — partition graph by minimal interconnection
* **Circuit design** — splitting components
* **Image segmentation** — separating regions
* **Community detection** — sparse connections between groups
...
2. Repeatedly run **Maximum Adjacency Search**:
   ...
   * Start from a vertex
   * Grow a set by adding the most tightly connected vertex
   * The last added vertex defines a cut
...
1. While ( |V| > 2 ):
   ...
   * Contract (u, v) into one node
   * Remove self-loops
2. Return number of edges between remaining nodes
...
* Build a **tree** where
  the **min cut between any two vertices** =
...
2. For each vertex (t \neq s),
   ...
   * Run max flow to find min cut between (s, t).
   * Partition vertices accordingly.
...
* Quickly answer all-pairs cut queries
* Network reliability
...
* **s–t min cut** = via **max flow**
* **global min cut** = min over all (s, t) pairs
...
* **Hopcroft–Karp** — fast matching in bipartite graphs
* **Hungarian Algorithm** — optimal weighted assignment
* **Edmonds’ Blossom Algorithm** — general graphs (non-bipartite)
...
* **Perfect Matching:** covers all vertices (each vertex matched once)
* **Maximum Weight Matching:** matching with largest total edge weight

**Graph Types:**

* **Bipartite:** vertices split into two sets (L, R); edges only between sets
* **General:** arbitrary connections (may contain odd cycles)
...
* Stable marriages (with preferences)
...
2. While augmenting paths exist:
   ...
   * BFS builds **layer graph** (shortest augmenting paths).
   * DFS finds all augmenting paths along those layers.
...
* When odd cycle encountered (blossom), **shrink it into one vertex**
* Continue search
* Expand blossoms at end
...
* Non-bipartite job/task assignments
* General pairing problems
* Network design
...
* Left → Source edges (capacity 1)
* Right → Sink edges (capacity 1)
* Between sets → edges (capacity 1)
...
* **Heavy-Light Decomposition (HLD)** — break trees into chains for segment trees / path queries
* **Centroid Decomposition** — recursively split tree by balance for divide-and-conquer
...
* `dfs_sz(u)` → compute subtree sizes
* `decompose(u, head)` → assign chain heads
...
* While heads differ, move up chain by chain
* Query segment tree in `[pos[head[u]], pos[u]]`
* When in same chain, query segment `[pos[v], pos[u]]`
...
1. **Find centroid**
   ...
   * Choose node where largest subtree ≤ n/2
2. **Decompose:**
...
* Distance queries (decompose + store distance to centroid)
* Tree problems solvable by divide-and-conquer
* Dynamic queries (add/remove nodes)
...
* HLD often uses **LCA** internally.
* Centroid decomposition may use **distance to ancestor** (via LCA).
* All exploit tree structure to achieve sublinear queries.
...
* **Topological Sorting & DAG DP**
* **Strongly Connected Components (Condensation Graphs)**
* **Articulation Points & Bridges (2-Edge/Vertex Connectivity)**
* **Eulerian & Hamiltonian Paths**
* **Graph Coloring & Bipartiteness Tests**
* **Cycle Detection & Directed Acyclic Reasoning**
* **Small-to-Large Merging, DSU on Tree, Mo’s Algorithm on Trees**
* **Bitmask DP on Graphs**
* **Dynamic Graphs (Incremental/Decremental BFS/DFS)**
* **Special Graphs (Planar, Sparse, Dense)**
...
* DP on DAG (longest/shortest path, counting paths)
...
* 2-edge/vertex connectivity tests
...
* Route reconstruction (e.g., word chains)
* Postman problems
...
* **DFS + recursion stack** for directed graphs
...
* Convert path queries to ranges via **Euler Tour**
* Use Mo’s ordering to process in O((N + Q)√N)
...
* **Incremental BFS:** maintain distances as edges added
* **Decremental connectivity:** union-find rollback or dynamic trees
...
* **Planar graphs:** ≤ 3V–6E; use face counting
* **Sparse graphs:** adjacency lists best
* **Dense graphs:** adjacency matrix / bitset
...
* **Overlapping subproblems:**
  The same smaller computations repeat many times.
...
* Transition: `dp[i] = dp[i-1] + dp[i-2]` (like stairs)
* Base: `dp[0] = 1`
...
* **Knapsack** (maximize value under weight constraint)
* **Subset Sum** (can we form a given sum?)
* **Coin Change** (how many ways or fewest coins to reach a total)
...
* If we **don’t take it**: `dp[i-1][w]`
* If we **take it** (if `w[i] ≤ w`): `dp[i-1][w - w[i]] + v[i]`
...
* Don’t take: `dp[i-1][sum]`
* Take (if `a[i] ≤ sum`): `dp[i-1][sum - a[i]]`
...
* Skip coin: `dp[i-1][t]`
* Take coin (unlimited): `dp[i][t - c[i]]`
...
* Knapsack: optimize under constraint
* Subset Sum: choose feasibility
* Coin Change: count or minimize
...
* If chars equal → take it and move diagonally
* Else move toward larger neighbor
...
* **LIS** builds upward — from smaller sequences.
* **LCS** aligns two sequences — compare prefixes.
* **Edit Distance** quantifies *difference* — minimal edits.
...
* **Polygon Triangulation** – divide shape into minimal-cost triangles
* **Optimal BST / Merge Patterns** – weighted merging decisions
...
* **Subset covering / visiting problems**
* **Permutations and combinations of sets**
* **Game states and toggles**
...
* A subset is represented by a mask from `0` to `(1 << n) - 1`.
...
* `mask & (1 << i)` → whether element `i` is in subset
  We can add elements:
...
* Each mask represents set of workers assigned.
* State: `dp[mask] = min cost for assigned tasks`.
...
* **Digit DP** – counting numbers with certain properties (e.g. digit sum, constraints)
* **SOS DP** (Sum Over Subsets) – efficiently computing functions over all subsets
...
* The sum of digits equals a target.
* The number doesn’t contain a forbidden digit.
* The number has certain parity or divisibility.
...
* `sum`: property tracker (e.g. sum of digits, remainder)
* `tight`: whether we’re still restricted by N’s prefix
* `leading_zero`: whether we’ve started placing nonzero digits
...
* **Sum over supersets**: reverse direction.
* **Max instead of sum**: replace `+=` with `max=`.
* **XOR convolution**: combine values under XOR subset relation.

---

### E. Applications

* Inclusion–exclusion acceleration
* Precomputing subset statistics
* DP over masks with subset transitions
...
* ( m_k ) is slope (depends on k)
* ( b_k = dp[k] + c(k) )
* ( x_i ) is known (monotonic)
...
* Slopes ( m_k ) are **monotonic** (either increasing or decreasing)
* Query points ( x_i ) are **sorted**
...
* `dp[u][0]` = ways if `u` is **not selected**
* `dp[u][1]` = ways if `u` **is selected**
...
* Number of ways to select subtrees
...
* **Store all predecessors** instead of one
* **Backtrack recursively** to enumerate all solutions
* **Tie-break deterministically** (e.g., lexicographically smallest)
...
* LCS: diagonal (↖), up (↑), left (←)
* Shortest path: parent edges
* LIS: predecessor chain
...
1. **State**: what subproblem you’re solving
   ...
   * Often `dp[i]`, `dp[i][j]`, or `dp[mask]`
   * Represents smallest unit of progress
...
* Fibonacci
* Knapsack (1D capacity)
* LIS (sequential dependency)
...
* Divisibility / digit sum problems
...
* Can transitions be reduced (monotonicity)?
* Can overlapping cost be cached (prefix sums)?
* Can dimensions be compressed (rolling arrays)?
...
* **Compress dimensions:** if only `dp[i-1]` needed, use 1D array.
* **Invert loops:** bottom-up ↔ top-down.
* **Change base:** prefix-sums for range queries.
* **State lifting:** add dimension for new property (like remainder, parity, bitmask).
...
* **Modular Arithmetic** (addition, subtraction, multiplication, inverse)
...
* Else, square ( s-1 ) times
* If none equal ( n - 1 ), composite
...
2. Count the number of 5-bit subsets with exactly 3 bits on.
3. Write a loop to print all subsets of `{a, b, c, d}`.
4. Use Lucas’ theorem for ( C(1000000, 1000) \mod 13 ).
...
* Drawing an ace from a deck: ( \frac{4}{52} = \frac{1}{13} )
...
* **Markov:** ( P(X \ge kE[X]) \le 1/k )
* **Chebyshev:** ( P(|X-E[X]| \ge c\sigma) \le 1/c^2 )
...
* For each prime ( p ), mark ( p \times i ) only once.
* Use `spf[i]` to store smallest prime factor.
...
* Get primes, SPF, and factorizations in ( O(n) ).
* Ideal for problems needing many factorizations.
...
1. Generate all primes ≤ 10^6 using a linear sieve.
2. Factorize 840 using the SPF array.
3. Compute φ(n) for n = 1..20.
4. Verify ( a^{φ(n)} \equiv 1 \pmod{n} ) for ( a=3, n=10 ).
5. Solve ( a^b \bmod n ) with ( b ) very large using φ(n).
...
* Row2 ← Row2 − ½ Row1 → ( [1, 2 | 5] \to [0, 0.5 | 1] )
* Back substitute → ( y = 2, x = 1 )
...
* Always pick **largest pivot** (partial pivoting)
* Avoid dividing by small numbers
* Use **EPS = 1e-9** threshold
...
* Cooley–Tukey FFT (complex numbers)
* NTT (modular version)
* Applications (polynomial multiplication, convolution)
...
* **Runge–Kutta (RK4):** solving differential equations
...
* Small step ( h ): better accuracy, more cost
* Large ( h ): faster, less stable
* Always check convergence (( |x_{n+1} - x_n| < \varepsilon ))
* Avoid division by small derivatives in Newton’s method
...
5. Experiment with step sizes ( h = 0.1, 0.01, 0.001 ) — observe convergence.
...
3. At each step:
   ...
   * Choose leaving variable (min ratio test).
   * Pivot to new vertex.
...
* **Stochastic Gradient Descent (SGD):** random subset of data
...
* **Linear Programming (Simplex):** logistics, scheduling
* **Quadratic Programming:** portfolio optimization
* **Gradient Methods:** ML, curve fitting
* **Convex Programs:** control systems, regularization
...
| ---------------- | ---------- | ---------------------------- | ------------------------ |
| Simplex          | Linear     | Polynomial avg               | LP solver                |
| Gradient Descent | Continuous | ( O(k) )                     | Needs step size          |
| Convex Methods   | Convex     | ( O(k \log(1/\varepsilon)) ) | Global optima guaranteed |
...
5. Experiment with learning rates: too small (slow), too large (diverge).
...
where ( Z[i] ) = length of longest substring starting at ( i ) that matches the prefix of ( P ).
...
* A set of patterns ( { P_1, P_2, \ldots, P_k } )
  with total length ( m = \sum |P_i| )
...
3. BFS over nodes:
   ...
   * For each edge ( (u, c) → v ):
     follow failure links from ( u ) until you find ( f ) with edge ( c )
     then ( v.\text{fail} = f.c )
...
* `fail("he") = root`
* `fail("hers") = "rs"` path invalid → fallback to `"s"` if exists
...
* Spam filters
* Intrusion detection (e.g., Snort IDS)
* Keyword search in compilers
* DNA sequence scanners
...
* **Suffix Array (SA)** – lexicographically sorted suffix indices
* **Longest Common Prefix (LCP)** array – shared prefix lengths between neighbors
...
* Construction in ( O(n) ) (Ukkonen’s algorithm)
* Pattern search in ( O(m) )
* Many advanced uses (e.g., longest repeated substring)
...
* `"abba"` is even-length palindrome
* `"aba"` is odd-length palindrome
...
3. For each position `i`:
   ...
   * initialize `P[i] = min(R - i, P[mirror])`
   * expand around `i` while characters match
   * if new palindrome extends past `R`, update `C` and `R`
...
* **Palindrome Queries:** is substring ( S[l:r] ) palindrome? → precompute radii
* **Longest Palindromic Substring**
* **DNA Symmetry Analysis**
* **Pattern Compression / Period Detection**
* **String Regularity Tests**
...
Let ( dp[i][j] ) = minimum edits to convert ( A[0..i-1] \to B[0..j-1] ).
...
* **Weighted Distance:** different operation costs
* **Local Alignment:** only best matching substrings
...
3. While heap size > 1:
   ...
   * Extract two smallest nodes `a`, `b`.
   * Create new node with `freq = a.freq + b.freq`.
   * Push back into heap.
...
* Read input, find **longest prefix** in dictionary.
* Output `(index, next_char)` and insert new entry.
...
* Simple **checksums** (parity, CRC)
* **Cryptographic hash functions** (MD5, SHA family, BLAKE3)
* Properties like **collision resistance** and **preimage resistance**
* Practical uses in verification, signing, and storage
...
* **Data integrity:** verify files (e.g., SHA256SUM)
* **Digital signatures:** sign hashes, not raw data
* **Password storage:** store hashes, not plaintext
* **Deduplication:** detect identical files via hashes
* **Blockchain:** link blocks with hash pointers
* **Git:** stores objects via SHA-1 identifiers
...
* The pattern might contain typos or mutations
* The text arrives in a stream (too large to store entirely)
* You want to match “close enough,” not “exactly”
...
* Near-duplicate detection (SimHash in Google)
* Network anomaly detection
* Real-time log matching
...
* **Graham Scan** – elegant and simple (O(n log n))
* **Andrew’s Monotone Chain** – robust and practical (O(n log n))
* **Chan’s Algorithm** – advanced and optimal (O(n log h), where h = number of hull points)
...
3. Traverse points and maintain a **stack**:
   ...
   * Add point
   * While last three points make a **right turn**, pop middle one
...
* **Closest Pair of Points** – find two points with minimum Euclidean distance
* **Segment Intersection** – determine if (and where) two line segments intersect
...
**Time Complexity:** ( O((n + k) \log n) ), where ( k ) = number of intersections.
...
* **Event sorting** → handle things in order
* **Active set maintenance** → track current structure
* **Updates and queries** → respond as the sweep progresses
...
* Treat **vertical edges** as events (entering/exiting rectangles).
* Sweep line moves along x-axis.
* Maintain y-intervals in active set (using a segment tree or interval tree).
* At each step, multiply **current width** × **height of union of active intervals**.
...
* Events are 2D cells or regions.
* Sweep front is a **plane** instead of a line.
...
* Draw bisectors between each pair.
* Intersection points define **Voronoi vertices**.
* Resulting polygons cover the plane, one per site.
...
* Neighboring cells share edges.
* The diagram’s **vertices** are centers of **circumcircles** through three sites.
...
* Used in finite element meshes, terrain modeling, and path planning.
* Leads to natural neighbor interpolation and smooth surfaces.
...
* Insert point
* Remove all triangles whose circumcircle contains point
* Reconnect the resulting cavity
...
* Horizontal edges (special treatment to avoid double counting)
...
* Triangles are simple, stable, and efficient for rendering and computation.
* Used in **graphics pipelines**, **area computation**, **physics**, and **mesh generation**.
...
1. Find an **ear**: a triangle formed by three consecutive vertices ( (v_{i-1}, v_i, v_{i+1}) )
   ...
   * It is **convex**
   * Contains **no other vertex inside**
...
* Which objects are near a given point?
* Which shapes intersect a region?
...
* Each level splits by a different axis (x, y, x, y, ...).
* Left child contains points with smaller coordinate.
* Right child contains larger coordinate.
...
* **Range query**: Find points in a region.
* **Nearest neighbor**: Search branches that might contain closer points.
* **K-nearest neighbors**: Use priority queues.
...
* Nearest neighbor in ML
* Collision detection
* Clustering (e.g., k-means acceleration)
...
* Store **objects or bounding boxes** in leaf nodes.
* Internal nodes store **MBRs** (Minimum Bounding Rectangles) that cover child boxes.
* Query by traversing overlapping MBRs.
...
* **Insert**: Choose subtree → Insert → Adjust MBRs
* **Search**: Descend into nodes whose MBR intersects query
* **Split**: When full, use heuristics (linear, quadratic, R*-Tree)
...
* **R*-Tree**: Optimized reinsertion, better packing
* **R+ Tree**: Non-overlapping partitions
* **Hilbert R-Tree**: Uses space-filling curves
...
* What color or depth should each pixel have?
...
* Lane detection in self-driving
* Shape recognition (circles, ellipses)
...
1. **Scale-space extrema detection**
   ...
   * Use **Difference of Gaussians (DoG)** across scales.
   * Detect maxima/minima in space-scale neighborhood.
...
* **ORB** (FAST + BRIEF): Efficient for real-time
* **SURF**: Faster SIFT alternative
* **Harris / FAST**: Corner detectors
* **Deep features**: CNN-based descriptors
...
* **A***: Heuristic search in grids and graphs
* **RRT (Rapidly-Exploring Random Tree)**: Sampling-based exploration
* **PRM (Probabilistic Roadmap)**: Precomputed navigation networks
...
* A **space** (grid, graph, or continuous)
* A **start node** and **goal node**
* A **cost function** (distance, time, energy)
* Optional **obstacles**
...
* Grow a tree from the **start** by randomly sampling points.
* Extend tree toward each sample (step size ( \epsilon )).
...
* Exact geometry often replaced by **approximations**.
* Volume, distance, and intersection tests become more expensive.
...
* **Exact arithmetic**: rational or symbolic computation
* **Grid snapping**: quantize space for stability
...
* A point ( (a, b) ) maps to line ( y = ax - b ).
* A line ( y = mx + c ) maps to point ( (m, -c) ).

Applications:

* Transforming **line intersection** problems into **point location** problems
* Simplifying half-plane intersections
* Enabling **arrangement algorithms** in computational geometry
...
* **Randomized incremental construction** (Convex Hulls, Delaunay)
* **Random sampling** for approximation (ε-nets, VC dimension)
* **Monte Carlo geometry** for probabilistic intersection and coverage
...
* 3D printing (watertightness)
* Data analysis (persistent homology)
* Robotics (free space topology)
...
* **K-means** → Voronoi partitioning
* **Manifold learning** → low-dim geometry
* **Convex optimization** → geometric feasibility
...
* **Point location** in dynamic settings
* **Visibility graphs** in complex polygons
* **Motion planning** in high dimensions
* **Geometric median / center** problems
* **Approximation guarantees** in robust settings
...
* Writers produce new versions; conflicts detected at **commit time**.
...
* **Durability** – once committed, always safe
...
* Use **coordinated checkpoints**
* Maintain **global commit logs**
* Consensus-based protocols (**Paxos Commit**, **Raft**) can replace 2PC for high availability
...
* **Responsive**: Short tasks don’t starve.
* **Downside**: Context switching overhead if quantum is too small.
...
* **Non-preemptive**: The CPU is released voluntarily.

#### Problems

* **Starvation**: Low-priority tasks may never run.
* **Solution**: **Aging** – gradually increase waiting task priority.
...
* **Preemptive SJF** (Shortest Remaining Time First): Preempts if a shorter job arrives.
...
* Queue 1: System → RR (quantum = 10ms)
* Queue 2: Interactive → RR (quantum = 50ms)
* Queue 3: Batch → FCFS (First-Come-First-Serve)
...
* CPU-bound → move down (lower priority)
* I/O-bound → move up (higher priority)
...
* **CPU**: L1, L2, L3 caches speed up memory access
* **Databases**: query results or index pages
* **Web browsers / CDNs**: recently fetched pages
* **Operating systems**: page cache for disk blocks
...
* **Stack (list)**: move item to top on access
* **Hash map + doubly linked list**: `O(1)` insert, delete, lookup
...
* **CAR (Clock with Adaptive Replacement)** – CLOCK-style adaptation
* **TinyLFU** – frequency sketch + admission policy
* **Hyperbolic caching** – popularity decay for large-scale systems
...
* OS uses it for **paging**
* Databases for **buffer pools**
* CPUs for **memory hierarchies**
* CDNs for **global acceleration**
...
* **Scalability**: support large, dynamic networks
* **Robustness**: recover from failures
* **Fairness**: avoid starving flows
...
* **Dynamic**: routes adjust automatically as topology changes (internet-scale)

#### Unicast, Multicast, Broadcast

* **Unicast**: one-to-one (most traffic)
* **Multicast**: one-to-many (video streaming, gaming)
* **Broadcast**: one-to-all (local networks)
...
* Builds shortest paths from one source
...
* Basis for **Distance-Vector** routing (RIP)
...
* **Slow Start**: probe capacity
* **Congestion Avoidance**: grow cautiously
* **Fast Retransmit / Recovery**

Modern variants:

* **TCP Reno**: classic AIMD
* **TCP Cubic**: non-linear growth for high-speed networks
* **BBR (Bottleneck Bandwidth + RTT)**: model-based control
...
* **RED (Random Early Detection)** – drop packets early
* **CoDel (Controlled Delay)** – monitor queue delay, drop adaptively
...
* **Control Plane**: decides routes (OSPF, BGP)
* **Data Plane**: forwards packets (fast path)

Modern networking (e.g. **SDN**, Software Defined Networking) separates these:

* **Controller** computes routes
* **Switches** act on flow rules
...
* **Data center networks** (with load balancing)
* **Cloud services** and **microservice meshes**
...
* **Validity**: the chosen value was proposed by a node
...
* **Randomization**, or
* **Partial synchrony** (timeouts, retries)
...
* **Safety**: no two different values chosen
* **Liveness**: possible under stable leadership
...
* **Leader**: coordinates all changes
...
1. **Leader Election**
   ...
   * Each follower votes; majority wins.
2. **Log Replication**

   * Leader appends entries, sends `AppendEntries` RPCs.
   * Followers acknowledge; leader commits when majority ack.
3. **Safety**

   * Logs are consistent across majority.
   * Followers accept only valid prefixes.
...
* Two quorums always **intersect**, ensuring consistency.
* Write quorum + read quorum ≥ total nodes.

In Raft/Paxos:

* Majority = `N/2 + 1`
* Guarantees overlap even if some nodes fail.
...
* Every node applies the same commands in the same order.
* Guarantees **deterministic, identical states**.

This model powers:

* **Databases** (etcd, Spanner, TiKV)
* **Coordination systems** (ZooKeeper, Consul)
...
* New elections restore progress

Algorithms:

* **Raft Election** (random timeouts)
* **Bully Algorithm**
...
* **Batching**: amortize RPC overhead
* **Pipeline**: parallelize appends
* **Read-only optimizations**: serve from followers (stale reads)
* **Witness nodes**: participate in quorum without full data

Advanced:

* **Multi-Paxos**: reuse leader, fewer rounds
* **Fast Paxos**: shortcut some phases
* **Viewstamped Replication**: Paxos-like log replication
...
* **etcd**, **Consul**, **ZooKeeper** – cluster coordination
* **Raft in Kubernetes** – leader election
* **PBFT in blockchains** – fault-tolerant ledgers
* **Spanner, TiDB** – consistent databases
...
* **Efficiency** – fully utilize servers
...
* Keeps cache locality
* Used in CDNs, distributed caches (e.g. memcached)
...
* Map servers to a **hash ring**
* A request’s key is hashed onto the ring
...
* **CDNs**
* **Distributed caches (Redis Cluster, DynamoDB)**
* **Load-aware systems**
...
* Heartbeat pings (HTTP/TCP)
* Auto-remove unhealthy servers
* Rebalance traffic instantly
...
* **Anycast**: advertise same IP globally; routing picks nearest
* **Latency-based routing**: measure and pick lowest RTT
...
* **Fairness issues** (no user dominates resources)
...
* Tokens refill at constant rate (rate limit)
* If empty → reject or delay
...
* Requests flow into a bucket, drain at fixed rate
* Excess = overflow = dropped

Smooths bursts; used for **shaping**.

#### Fixed Window Counter

* Count requests in fixed interval (e.g. 1s)
* Reset every window
* Simple but unfair around boundaries

#### Sliding Window Log / Sliding Window Counter

* Maintain timestamps of requests
* Remove old ones beyond time window
...
* Use **rate limiting** per user or service
* Use **load balancing** across nodes
* Apply **circuit breakers** when overload persists
...
* **Stable** – prevent cascading failures
* **Fair** – each client gets a fair share
* **Resilient** – recover gracefully from spikes or node loss
...
* A **query** (e.g., “machine learning algorithms”)

We want to return:

* **Relevant** documents
* **Ranked** by **importance** and **similarity**
...
* Query = Boolean expression
  e.g. `(machine AND learning) OR AI`
...
* ( k_1, b ): tunable params (typ. 1.2–2.0, 0.75)

#### Advantages

* Balances **term frequency**, **document length**, and **rarity**
* Fast and effective baseline
* Still used in **Elasticsearch**, **Lucene**, **OpenSearch**
...
* Each term has upper-bound score
* Maintain pointers in each posting
* Compute potential max score
* If max < current threshold, **skip doc**

Improves latency for **top-k retrieval**.

Variants:

* **BMW (Block-Max WAND)** – uses block-level score bounds
* **MaxScore** – simpler thresholding
* **Dynamic pruning** – skip unpromising candidates
...
* **Delta encoding**: store gaps between docIDs
* **Variable-byte (VB)** or **Gamma coding**
* **Frame of Reference (FOR)** and **SIMD-BP128** for vectorized decoding
...
* **Learning to Rank (LTR)**: use features (tf, idf, BM25, clicks)
* **Neural re-ranking**: BERT-style embeddings for semantic similarity
* **Hybrid retrieval**: combine BM25 + dense vectors (e.g. **ColBERT**, **RRF**)
...
* Web search engines
* IDE symbol lookup
* Log search, code search
* Database full-text search
* AI retrieval pipelines (RAG)
...
* **Databases** – column stores, indexes, logs
* **Networks** – HTTP, TCP, QUIC payloads
* **File systems** – ZFS, NTFS, btrfs compression
* **Streaming** – video/audio codecs
* **Logs & telemetry** – reduce I/O and storage cost

Benefits:

* Smaller data = **faster I/O**
* Less storage = **lower cost**
* Less transfer = **higher throughput**

Trade-offs:

* CPU overhead (compression/decompression)
* Latency (especially for small data)
* Suitability (depends on entropy and structure)
...
* **Lossless**: reversible (ZIP, PNG, LZ4)
* **Lossy**: approximate (JPEG, MP3, H.264)
...
* Optimal under symbol-level model

Used in: **DEFLATE**, **JPEG**, **MP3**

#### Arithmetic Coding

* Encodes sequence as fractional interval
* More efficient than Huffman for skewed distributions
* Used in: **H.264**, **bzip2**, **AV1**

#### Dictionary-Based (LZ77, LZ78)

* Replace repeated substrings with references
* Core of **ZIP**, **gzip**, **zlib**, **LZMA**, **Snappy**
...
* **Zstandard (Zstd)** – tunable speed/ratio, dictionary support

#### Burrows–Wheeler Transform (BWT)

* Reorders data to group similar symbols
* Followed by Move-To-Front + Huffman
* Used in **bzip2**, **BWT-based compressors**

#### Run-Length Encoding (RLE)

* Replace consecutive repeats with (symbol, count)
* Great for structured or sparse data
...
* **Dictionary encoding** – map strings → ints
* **Run-length encoding** – good for sorted columns
* **Delta encoding** – store differences (time series)
* **Bit-packing** – fixed-width integers in minimal bits
...
* Structured formats → fieldwise encoding
* Often **Snappy** or **LZ4** for fast decode
* Aggregators (Fluentd, Loki, Kafka) rely heavily on them

#### Data Lakes and Files

* **Parquet**, **ORC**, **Arrow** → columnar + compressed
* Choose codec per column: LZ4 for speed, Zstd for ratio
...
* Enables random access and parallelism
* Needed for **network streams**, **distributed files**
...
* 33% overhead
* Used for binary → text (emails, JSON APIs)
...
* Compact integers (e.g. protobufs)
...
* **Context mixing** (PAQ)
* **Zstd** uses FSE (Finite State Entropy) coding
...
* **CPUs** with SIMD (AVX2, SSE4.2)
* **GPUs** (parallel encode/decode)
* **NICs / SmartNICs**
* **ASICs (e.g., Intel QAT)**
...
* Saves petabytes in data centers
* Boosts throughput across networks
* Powers cloud storage (S3, BigQuery, Snowflake)
* Enables efficient analytics and ML pipelines
...
* **Availability** – keep serving requests
...
* **Hardware**: multiple power supplies, disks (RAID)
* **Software**: replicated services, retries
* **Data**: multiple copies, erasure codes
* **Temporal**: retry or checkpoint and replay
...
* On restart, **replay log** of missed events
...
* **Timeouts** – suspect node if no heartbeat
* **Gossip protocols** – share failure info among peers
...
* **Self-healing clusters** (Kubernetes)
* **Rebalancing** (Cassandra ring repair)
...
* **Databases** (replication + WAL)
* **Distributed storage** (HDFS, Ceph, S3)
...
* **Multinomial NB** – text (bag of words)
...
* Slow on large data
* Harder to tune kernel parameters
...
* Pick feature ( f ) and threshold ( t )
...
* Reduces variance
* Works well with high-variance learners
* Parallelizable
...
* Needs decay schedule for stability
...
* **Tagging** (POS tagging, named entity recognition)
* **Transcription** (speech → text)
* **Decoding** (translation, path reconstruction)
...
* A sequence of **hidden states** ( z_1, z_2, \ldots, z_T )
* Each state emits an observation ( x_t )
* Transition and emission probabilities govern the process
...
* Larger beam → better accuracy, slower
* Smaller beam → faster, riskier
...
* **Viterbi**: POS tagging, speech decoding
* **Beam Search**: translation, text generation
* **CTC**: ASR, OCR, gesture recognition
...
* In beam search, apply **length normalization**
* In CTC, use **dynamic programming tables**
* Combine CTC + beam search for speech decoding

---

### 9. Common Pitfalls

* **Viterbi** assumes Markov property (limited memory)
* **Beam Search** can miss global optimum
* **CTC** can confuse repeated characters without blanks
...
* Continuous optimization (parameter tuning)
...
* Define neighbor / mutation operators
* Balance randomness and greediness
...
* **GA + SA**: evolve population, fine-tune locally
* **PSO + DE**: use swarm + differential evolution
* **ACO + Local Search**: reinforce with hill-climbing
...
* Poor representation → weak search
* Over-exploitation → stuck in local optima
* Bad parameters → chaotic or stagnant behavior
...
Repeat while exploring (e.g. **ε-greedy**):
...
* **Exploration**: try new actions to gather knowledge
* **Exploitation**: use current best knowledge to maximize reward
...
* Critic: estimates value (baseline)
...
* **Deep Q-Networks (DQN)**: use neural nets for Q(s, a)
* **PPO / A3C**: advanced actor-critic methods
* **TD(λ)**: tradeoff between MC and TD learning
...
* **Approximation algorithms**: Solve **NP-hard** problems with provable bounds.
* **Online algorithms**: Make **immediate decisions** without knowing the future.
...
* Else → miss, must evict one page

**Strategies:**

* **LRU (Least Recently Used):** evict oldest
* **FIFO:** evict first loaded
...
* **Streaming algorithms:** One pass, small memory (Count-Min, reservoir sampling)
* **Online learning:** Update models incrementally (SGD, perceptron)
* **Approximate dynamic programming:** RL and heuristic search
...
* **Historical bias** (biased data)
* **Measurement bias** (imprecise features)
* **Selection bias** (skewed samples)
...
* **States** ( S )
* **Actions** ( A )
* **Transition function** ( T(s, a) \to s' )
* **Goal condition** ( G \subseteq S )
* **Cost function** ( c(a) )
...
* **HTN (Hierarchical Task Networks):**
  Define high-level tasks broken into subtasks.
...
* **MDPs:** full observability, stochastic transitions
...
* **Search** (planning backbone)
* **Learning** (policy, heuristic, model)
* **Simulation** (data generation)

Examples:

* **AlphaZero:** self-play + MCTS + neural nets
* **MuZero:** learns model + value + policy jointly
* **Large Language Agents:** use reasoning + memory + search

---

### 9. Real-World Applications

* **Robotics:** motion planning, pathfinding
* **Games:** Go, Chess, strategy games
* **Logistics:** route optimization
* **Autonomy:** drones, vehicles, AI assistants
* **Synthesis:** program and query generation
...
* **Planning** sequences actions toward goals
* **Learning** adapts heuristics from experience
